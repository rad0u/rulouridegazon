...
// Parcele: Insamantare. Mai trebuie o noua baza de date, pe langa cea cu
// fertilizanti - materii prime in care deocamdata trecem seminte gazon,
// urmand sa incarc gestiunile fermelor cu cantitatea de seminte de gazon" —
// acest ecran este o oglindă exactă a app/substante/SubstanteScreen.tsx
// (nomenclator + alimentare gestiune fermă + stoc curent + istoric/corectare
// alimentări), dar pe tabelele materii_prime* (separate de substante*), ca
// gestiunile de fertilizanți și de semințe să nu se amestece.

type NomenclatorItem = { id: string; nume: string; unitate_masura: string };
type Ferma = { id: string; nume: string };

type MateriePrimaStoc = {
  id: string;
  nume: string;
...
export default function MateriiPrimeScreen() {
  ...
  if (role === 'admin_central') return <MateriiPrimeAdminCentral />;
  if (role === 'admin_ferma') return <MateriiPrimeAdminFerma />;
...
function MateriiPrimeAdminCentral() {
  ...
  async function incarca() {
    ...
    const [nomenclatorRes, fermeRes, stocuriRes, intrariRes] = await Promise.all([
      supabase.from('materii_prime_nomenclator').select('id, nume, unitate_masura').order('nume'),
      ...
      supabase
        ...
        .order('nume'),
      supabase
        .from('materii_prime_intrari')
        .select(
          'id, cantitate, pret_intrare_unitar, data, furnizor, nota, created_at, materii_prime(nume, unitate_masura, ferma_id, ferme(nume)), utilizatori(nume)',
        )
    ...
    setStocuri((stocuriRes.data as unknown as MateriePrimaStoc[]) ?? []);
  ...
  async function adaugaNomenclator(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!nomenclatorForm.nume.trim()) {
      setNomenclatorError('Completează denumirea materiei prime.');
      return;
    }

    setNomenclatorSaving(true);
    const { error: insertError } = await supabase.from('materii_prime_nomenclator').insert({
    ...
    if (insertError) {
      setNomenclatorError(
        insertError.code === '23505'
          ? 'Există deja o materie primă cu această denumire în nomenclator.'
          : insertError.message,
      );
      return;
    }
  ...
  const materiePrimaSelectata = nomenclator.find((n) => n.id === form.nomenclator_id) ?? null;
  ...
  async function salveazaAlimentare(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!form.nomenclator_id) {
      setSaveError('Alege materia primă din nomenclator.');
      return;
    }
    ...
    setSaving(true);
    const { error: rpcError } = await supabase.rpc('alimenteaza_materie_prima', {
  ...
  function deschideEditareIntrare(i: Intrare) {
    ...
    setEditIntrareForm({
      ferma_id: i.materii_prime?.ferma_id ?? '',
  ...
  async function salveazaEditareIntrare(intrareId: string) {
    ...
    const { error: rpcError } = await supabase.rpc('editeaza_alimentare_materie_prima', {
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.5rem' }}>
        <h1 style={{ margin: 0 }}>Materii prime</h1>
      ...
      <p style={{ color: '#666', marginTop: '-0.75rem' }}>
        Gestiune separată de Substanțe — pentru materii prime folosite direct în lucrare (deocamdată semințe de
        gazon, la operațiunea „Însămânțare” din Activități parcele).
      </p>
      ...
      <section style={{ border: '1px solid #eee', borderRadius: '8px', padding: '1rem' }}>
        <h2 style={{ marginTop: 0, fontSize: '1.1rem' }}>Nomenclator materii prime</h2>
        <p style={{ color: '#666', marginTop: '-0.5rem' }}>
          Catalogul comun de materii prime (denumire + unitate de măsură), folosit la alimentarea gestiunilor
          fermelor.
        </p>
      ...
      <section style={{ border: '1px solid #eee', borderRadius: '8px', padding: '1rem' }}>
        ...
        <p style={{ color: '#666', marginTop: '-0.5rem' }}>
          Adaugă stoc dintr-o materie primă din nomenclator la gestiunea unei ferme, cu prețul de intrare. Prețul
          mediu se recalculează automat (medie ponderată cu stocul existent).
        </p>
        <form onSubmit={salveazaAlimentare} style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', alignItems: 'flex-end' }}>
          ...
          <label>
            Cantitate {materiePrimaSelectata ? `(${materiePrimaSelectata.unitate_masura})` : ''}
            <input
              type="number"
              min="0"
              step="0.01"
          ...
          <label>
            Preț intrare (lei/{materiePrimaSelectata ? materiePrimaSelectata.unitate_masura : 'u.m.'})
            <input
              type="number"
              min="0"
              step="0.01"
        ...
        <table style={{ borderCollapse: 'collapse', width: '100%', fontSize: '0.9rem' }}>
          <thead>
            <tr style={{ textAlign: 'left', borderBottom: '1px solid #ddd' }}>
              ...
              <th style={{ padding: '0.4rem' }}>Materie primă</th>
            ...
            {loadedOnce && stocuri.length === 0 && (
              <tr>
                <td colSpan={5} style={{ padding: '0.75rem', color: '#666' }}>
                  Nicio materie primă în gestiune încă — alimentează prima din formularul de mai sus.
                </td>
              </tr>
            )}
            {stocuri.map((s) => (
              ...
              return (
                <Fragment key={i.id}>
                  ...
                  >
                    ...
                    <td style={{ padding: '0.4rem' }}>{i.materii_prime?.ferme?.nume ?? '—'}</td>
                    <td style={{ padding: '0.4rem' }}>{i.materii_prime?.nume ?? '—'}</td>
                    <td style={{ padding: '0.4rem' }}>
                      {i.cantitate} {i.materii_prime?.unitate_masura ?? ''}
                    </td>
                    <td style={{ padding: '0.4rem' }}>
                      {formatLei(i.pret_intrare_unitar)}/{i.materii_prime?.unitate_masura ?? ''}
                    </td>
                    ...
                    <tr>
                      <td colSpan={8} style={{ padding: '0.75rem', background: '#fafafa' }}>
                        ...
                        <p style={{ fontSize: '0.8rem', color: '#666', marginTop: '0.5rem', marginBottom: 0 }}>
                          Materia primă nu se poate schimba aici. Corectarea recalculează automat stocul și prețul
        ...
        >
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem 1.25rem', borderBottom: '1px solid #eee' }}>
              <h2 style={{ margin: 0, fontSize: '1.1rem' }}>Nomenclator materii prime ({nomenclator.length})</h2>
              <button
                onClick={() => setNomenclatorModalDeschis(false)}
...
function MateriiPrimeAdminFerma() {
  const [stocuri, setStocuri] = useState<MateriePrimaStoc[]>([]);
  const [intrari, setIntrari] = useState<Intrare[]>([]);
  const [loading, setLoading] = useState(false);
  const [loadError, setLoadError] = useState<string | null>(null);
  ...
  async function incarca() {
    ...
    const [stocuriRes, intrariRes] = await Promise.all([
      ...
      supabase
        .from('materii_prime_intrari')
        .select(
          'id, cantitate, pret_intrare_unitar, data, furnizor, nota, created_at, materii_prime(nume, unitate_masura, ferme(nume)), utilizatori(nume)',
        )
        .order('created_at', { ascending: false })
        .limit(50),
    ]);
            ...
            {loadedOnce && stocuri.length === 0 && (
              <tr>
                <td colSpan={4} style={{ padding: '0.75rem', color: '#666' }}>
                  Nicio materie primă în gestiune încă.
                </td>
              </tr>
            )}
            {stocuri.map((s) => (
